    ...
    var terms = new Bloodhound({
      ...
      remote: {
        ...
        prepare:  function(search_terms, settings) {
          ...
          // Settings contains { url: suggest_url, method: 'get', format: 'json }
          // Need to return with the URL modified with the given query.
          ...
          // Determine which search type has been selected on the menu.
          // If the type has changed, clear the cache so that the appropriate
          // set of information is searched rather than accepting cached values
          // that probably are not appropriate.
          //
          // NOTE: Is there a way to save and restore the cache?
          // If so, the cache could be saved when switching to a new search
          // type and then restored when that search type is selected again.
          //
