    ...
    def send_and_receive(path, solr_params = nil)
      ...
      benchmark('Solr fetch', level: :debug) do
        ...
        cfg.response_model.new(rsolr_response, solr_params, opt).tap do |r|
          verbose = Blacklight.logger.debug?
          verbose &&= defined?(BLACKLIGHT_VERBOSE_LOGGING)
          verbose &&= BLACKLIGHT_VERBOSE_LOGGING
          Blacklight.logger.debug { "Solr response: #{r.inspect}" } if verbose
        end
      end
...
=begin # NOTE: using base version
    ...
    #
    # This method overrides:
    # @see Blacklight::Solr::Repository#build_connection
    #
    def build_connection
      adapter = connection_config[:http_adapter]
      RSolr.connect(connection_config.merge(adapter: adapter))
    end
=end
    ...
    def solr_send_and_receive(http, path, solr_params)
      logger.debug { "Solr query: #{http} #{path} #{solr_params.inspect}" }
    ...
    def merge_url_params!(solr_params, url_params = nil, sb_filters = nil)
      if url_params.present?
        ...
        unless url_params.is_a?(sb_class)
          ...
          sb =
            if sb_filters.is_a?(Array) # NOTE: 0% coverage for this case
