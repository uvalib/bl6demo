...
module Blacklight::BlacklightHelperBehaviorExt
  ...
  #   @option options [Symbol] :title_tag
  #   @option options [Symbol] :author_tag
  #
  ...
  def document_index_view_type(query_params = nil)
    ...
    if view_param && document_index_views.keys.include?(view_param) # NOTE: 0% coverage for this case
      view_param
    else
  ...
  def document_title(doc = nil, opt = nil)
    doc ||= @document
    opt = opt ? opt.dup : {}
    alt_title = blacklight_config(doc).view_config(:show).alt_title_field
    opt[:format]              = false unless opt.key?(:format)
    opt[:show_title]          = false if doc.has?(alt_title)
