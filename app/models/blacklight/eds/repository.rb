...
=end
    ...
    #
    def connection
      raise caller.join("\n") unless Rails.env.production? # TODO: testing - remove
      self
    end
    ...
    # send_and_receive
    #
    # @param [Blacklight::SearchBuilder, ActionController::Parameters, Hash] search
    # @param [Hash] eds_params
    #
    # @return [Blacklight::Eds::Response]
    #
      ...
      benchmark('EDS fetch', level: :debug) do
        ...
        search =
          case search
            ...
            when ActionController::Parameters then search.to_unsafe_h
            when Hash                         then search # NOTE: 0% coverage for this case
          end
...
=begin # NOTE: using base version
    ...
    #
    # This method overrides:
    # @see Blacklight::AbstractRepository#connection_config
    #
...
=end
    ...
    def eds_get_record(id, eds_params = nil)
      ...
      eds_session('bl-repo-find', eds_params).retrieve(dbid: dbid, an: an)
        .tap { |result| $stderr.puts ">>> #{__method__}: #{result.to_solr['response']['numFound']}" } # TODO: debugging - remove
        #.tap { |result| $stderr.puts ">>> #{__method__}: #{result.to_solr.pretty_inspect}" } # TODO: debugging - remove
    end
...
=begin # TODO: debugging - remove
        .tap { |session|
          $stderr.puts "EDS info: #{session.info.pretty_inspect}" # TODO: debugging - remove
        }
=end
    end
